import {
  ...
  import { api } from '@/backendAPI/backend'; // <-- import the raw axios instance
  ...
  export default function HomeScreen() {
    ...
    useEffect(() => {
        const lookupUserId = async () => {
          try {
            ...
            const data: { user_id: string; username: string } =
              await backendGet(`/user/username/${username}`);
    ...
    const fetchPair = async (questionId: string) => {
        ...
        try {
          ...
          const data: Answer[] = await backendGet(`/question/${questionId}/get_pair`);
    ...
    const fetchLeaderboard = async (questionId: string) => {
        ...
        setLeaderboardError('');
        try {
          console.log(`▶️ fetchLeaderboard: calling /question/${questionId}/answer_leaderboard`);
          ...
          if (axios.isAxiosError(err) && err.response) {
            ...
            setLeaderboardError(serverMsg || `Server returned ${err.response.status}`);
          } else {
    ...
    const handleVote = async (answerId: string) => {
        ...
        try {
          ...
          console.log("◀️ handleVote: response status", resp.status, "data:", resp.data);
          ...
          if (isAxiosError(err) && err.response) {
            ...
            setPairError(
              typeof err.response.data === "object" 
                ? (err.response.data as any).error || `Server ${err.response.status}` 
